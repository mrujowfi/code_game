...
def multiArea(temp_house, block_line, block_row):
    ...
    try:
        ...
        result = travelBlock(block_now, selected_list)
...
def travelBlock(block_now, selected_list):
    global arg, line, row, house, house_list, start_line, start_row, history, failed_set, driver, num_map
    ...
    if i > 0 and block_now[i-1][j] == -1:
        #up
        block_now[i-1][j] = -1 - (depth+1)
        selected_list.append((i-1, j, depth+1))
    if i+1 < line and block_now[i+1][j] == -1:
        #down
        block_now[i+1][j] = -1 - (depth+1)
        selected_list.append((i+1, j, depth+1))
    if j > 0 and block_now[i][j-1] == -1:
        #left
        block_now[i][j-1] = -1 - (depth+1)
        selected_list.append((i, j-1, depth+1))
    if j+1 < row and block_now[i][j+1] == -1:
        #right
        block_now[i][j+1] = -1 - (depth+1)
        selected_list.append((i, j+1, depth+1))
    ...
    return travelBlock(block_now, selected_list)
